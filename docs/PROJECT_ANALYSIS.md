...
当前项目需要明确区分“监听地址”和“访问策略”：

- 监听地址由 [`app/server/core/env.js`](../app/server/core/env.js) 中的 `BIND_ADDR` 控制，默认是 `0.0.0.0`
- 访问策略由 [`app/server/services/management-access.js`](../app/server/services/management-access.js) 与 [`app/server/management-api.js`](../app/server/management-api.js) 配合控制

这意味着：

- 管理进程默认会在所有网卡上监听 `18790`
- 但如果“管理访问”收敛为仅本机，非回环来源即使能连到端口，也会在应用层被拒绝
- 因此“拒绝连接”和“403 forbidden”不是同一种问题，前者偏网络连通性，后者偏应用访问策略

结合当前产品决策，可以把访问链路分成三类：

- fnOS 默认中继：通常表现为平台已处理外部访问，再由本机管理服务承接，因此用户感知上最稳定
- 局域网直连 HTTP：要求端口可达，且管理访问允许远程来源
- frp/frps 反向代理：只要回源配置正确，通常也能访问；若回源在服务端侧表现为本机请求，则即使策略是仅本机也可能放行

这个设计并不提供本地账号/密码认证。项目当前明确依赖网络边界、反向代理边界和“管理访问”开关来限定暴露面。
