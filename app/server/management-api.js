...
const CONFIG_FILE = path.join(TRIM_PKGVAR, 'data', '.openclaw', 'openclaw.json');
...
const OC_BIN = path.join(TRIM_APPDEST, 'server', 'openclaw_global', 'bin', 'openclaw');
...
async function getStatus() {
  ...
  // 获取版本信息
  try {
    const packageJson = readJSON(path.join(TRIM_APPDEST, 'server', 'openclaw_global', 'lib', 'node_modules', 'openclaw', 'package.json'));
