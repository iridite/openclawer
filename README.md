...
管理服务默认监听 `0.0.0.0:18790`，也就是所有网卡都会接收连接；但“是否允许非本机来源访问”仍由管理访问策略决定。这两件事不是同一个层面：

- `BIND_ADDR=0.0.0.0`：解决的是“进程监听在哪些网卡上”
- WebUI「系统 -> 管理访问」：解决的是“收到请求后，是否允许非回环地址继续访问面板/API”

几种常见访问方式的语义如下：

- fnOS 默认中继访问：通常可以正常打开，因为请求会先经过 fnOS 提供的中继/转发链路，再回到本机管理服务
- 局域网直连访问：例如 `http://NAS_IP:18790`，要求 18790 端口在局域网内可达，且“管理访问”处于“允许远程访问”
- frp/frps 反向代理访问：只要代理能正确回源到 `127.0.0.1:18790` 或宿主机监听地址，通常也可以正常访问；如果回源最终表现为本机请求，即使管理访问收敛到仅本机，也可能通过
...
- 如果默认开放远程访问，请自行确保局域网、反向代理和公网暴露面处于可信范围
...
- 默认允许远程访问管理面板/API（进程仍监听 `0.0.0.0`，请确保网络环境可信）
- 如需收敛访问面，可在 WebUI 的「系统」->「管理访问」中切换为仅本机访问；切换后非回环来源会收到 `403 forbidden`
